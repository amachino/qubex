...
## 設計思想: Experimentalist-Friendly Interface

Qubex は、プログラミングの専門家ではない実験者が、Jupyter Notebook 等を用いてインタラクティブに量子実験を行うことを主眼に設計されている。

*   **All-in-One**: 必要な設定ファイルを用意した後は、`ex = Experiment(...)` で Experiment オブジェクトを作成するだけで、実験に必要な全ての機能（測定、キャリブレーション、解析など）に `ex` 経由でアクセスできること。
*   **Low Barrier**: 実験の都度、専用のクラスを import したり、複雑なカスタムクラスを記述したりする必要がないこと。

一方、この「使いやすさ」の反面、以前の実装では機能追加のたびに `Experiment` クラスが肥大化し、保守性が低下するという問題（God Class）が発生していた。
今回のリファクタリングの目的は、**このユーザー体験（シンプルなインターフェース）を維持しつつ**、内部構造を整理して保守性と拡張性を高めることにある。
