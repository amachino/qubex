...
リリース候補版 `1.5.0rc3` は、PyPI から
`pip install "qubex==1.5.0rc3"` でインストールできます。
実機用には `qubex[backend]==1.5.0rc3`（QuEL-1、Linux）または
`qubex[quel3]==1.5.0rc3` を指定してください。
正式版 v1.5.0 に向けた検証用のプレリリースです。
以下の Git を使うコマンドはリポジトリの版をインストールします。
