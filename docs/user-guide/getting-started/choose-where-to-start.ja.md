...
- 実機を使う `Experiment` では、続けて [システム設定](system-configuration.md) を用意します。
...
1. [`QuantumSimulator` クラス](../simulator/index.md) で扱える範囲を確認する
...
このセクションは、`Experiment` の facade の下でモジュール単位に扱いたい場合の
入口です。構成は次の 3 つです。

- [`measurement`](../measurement/index.md): `MeasurementSchedule`、キャプチャ/読み出し、sweep、`measurement` の実行フロー
- [`system`](../system/index.md): 設定読み込み、system model、同期処理
- [`backend`](../backend/index.md): backend controller、execution request、QuEL 固有 runtime
...
2. 必要に応じて [パルスシーケンスの組み方](../pulse-sequences/index.md) を読む
3. 目的に合うモジュールページを選ぶ
4. 対応するサンプルワークフローから notebook を始める: [`measurement`](../measurement/examples.md)、[`system`](../system/examples.md)、[`backend`](../backend/examples.md)

## 補助的に読むページ

- [パルスシーケンスの組み方](../pulse-sequences/index.md): `Experiment`、`QuantumSimulator`、低レベル実行スタックで共通して使う `PulseSchedule` の考え方を整理したいときに読みます。
