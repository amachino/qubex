...
| --- | --- | --- |
| [`measurement`](../measurement/index.md) | `MeasurementSchedule`、キャプチャ/読み出し、sweep、結果変換 | `measurement` モジュールの実行フローを直接扱いたい |
...
2. [`measurement`](../measurement/index.md) がその状態を使って、
   `MeasurementSchedule`、キャプチャ/読み出し、sweep のフローを構築します。
3. [`backend`](../backend/index.md) の controller が、準備された request を
   実際の QuEL ランタイムへ流します。

## 推奨される導線

- [`measurement`](../measurement/index.md):
  `MeasurementSchedule`、キャプチャ/読み出し、sweep、関連する実行フローを扱うならここから始め、
