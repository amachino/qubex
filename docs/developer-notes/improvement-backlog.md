...
| IB-006 | Compatibility contract guardrail | Require `Measurement` contract test updates for any compatibility-surface changes. | Prevent silent contract drift during refactors. | during v1.5.x | proposed |
...
| IB-009 | Pulse scheduling mixed-`dt` robustness | Implement per-channel `sampling_period` support in `PulseSchedule` with integer-tick normalization and LCM-cap guard for barrier alignment. | Avoid float precision drift and pathological blank growth when channel sampling periods differ. | after v1.5.0 GA | proposed |
| IB-010 | qxschema measurement protocol publication | Define and publish versioned `qxschema` contracts for `MeasurementConfig`/`MeasurementResult` and sweep counterparts with explicit converter guarantees. | Stabilize external integration while preserving internal runtime flexibility. | after v1.5.0 GA | proposed |
| IB-011 | Data persistence integration | Add raw-result persistence coverage, run manifests, xarray semantic export, and optional Tiled/Zarr integration as staged follow-up work. | Support Labber-like raw-data retention and external catalog workflows without making heavy storage behavior the default. | v1.6 planning | proposed |
