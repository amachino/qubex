...
- Target-to-alias mapping is configured explicitly (`instrument_alias_map`); fallback to target label is not allowed.
...
3. Execute once through `Measurement.run_measurement_schedule(schedule, config)`.
