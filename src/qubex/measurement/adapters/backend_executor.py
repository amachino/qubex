...
class Quel3BackendExecutor:
    ...
    def execute(self, *, request: BackendExecutionRequest) -> BackendExecutionResult:
        ...
        execute_impl = getattr(self._backend_controller, "execute_quel3_measurement", None)
        if not callable(execute_impl):
            raise TypeError(
                "Quel3 backend execution requires backend_controller.execute_quel3_measurement(payload=...)."
            )
