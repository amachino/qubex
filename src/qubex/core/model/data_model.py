...
_TYPE_NUMPY_NDARRAY: Final[str] = TYPE_NUMPY_NDARRAY
_TYPE_TUNITS_VALUE_ARRAY: Final[str] = TYPE_TUNITS_VALUE_ARRAY
...
class DataModel(Model):
    ...
    def save_netcdf(self, path: str | Path) -> Path:
        ...
        payload = _encode_value(self._raw_field_data(), arrays, path=())
        payload[META_KEY] = {
        ...
        with _open_netcdf(path_obj, mode="w") as ds:
            ds.setncattr(_ATTR_FORMAT, self.format_name)
            ds.setncattr(_ATTR_FORMAT_VERSION, self.format_version)
            ds.setncattr(
                _ATTR_MODEL_CLASS,
    ...
    def load_netcdf(cls, path: str | Path) -> Self:
        ...
        with _open_netcdf(path_obj, mode="r") as ds:
            _validate_netcdf_format(
                ds,
                expected_format=cls.format_name,
...
def _encode_value(
    ...
    *,
    ...
    if isinstance(value, np.ndarray):
        ...
        return {
            _VARIABLE_REF_KEY: ref,
        }
    if isinstance(value, tunits.ValueArray):
...
def _decode_value(value: Any, ds: Dataset) -> Any:
    if isinstance(value, dict):
        ...
        if _is_variable_ref(ref):
            ...
            if array_type == _TYPE_TUNITS_VALUE_ARRAY:
