...
@dataclass
class ReadoutFrequencyData(TargetData):
    """
    Readout-frequency calibration result for one target.
    ...
    target : str
        Target qubit label.
    data : NDArray
        Measured readout response values.
    frequency_range : NDArray
        Readout frequencies in GHz.
    readout_frequency : float
        Selected readout frequency in GHz.
    fit_func : str
        Fit model used for calibration.
    fit_result : FitResult
        Fit result used to select the readout frequency.
    peak_frequency : float, optional
        Maximum-response frequency in GHz.
    peak_value : float, optional
        Maximum measured response.
    figure : go.Figure, optional
        Figure generated by the fit or fallback plot.
    """

    frequency_range: NDArray
    readout_frequency: float
    fit_func: Literal["lorentzian", "double_lorentzian"]
    fit_result: FitResult
    ...
    def plot(
        ...
        *,
        ...
        """Plot the readout-frequency calibration result."""
        fig = self.figure
        if fig is None:
            fig = viz.make_figure()
            finite_mask = np.isfinite(self.frequency_range) & np.isfinite(self.data)
            fig.add_trace(
                go.Scatter(
                    x=self.frequency_range[finite_mask],
                    y=self.data[finite_mask],
                    mode="lines+markers",
                    name="Data",
                )
            )
            fig.update_layout(
                title=f"Readout frequency calibration : {self.target}",
                xaxis_title="Readout frequency (GHz)",
                yaxis_title="Rabi amplitude (arb. units)",
            )

        fig.show(config=get_config(filename=f"readout_frequency_{self.target}"))
        if return_figure:
            return fig

    def fit(self) -> FitResult:
        """Return the stored readout-frequency fit result."""
