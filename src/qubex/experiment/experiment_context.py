...
class ExperimentContext:
    ...
    def print_environment(self, verbose: bool | None = None) -> None:
        ...
        if verbose:
            logger.info(f"  qubex-core: {get_version('qubex-core')}")
            logger.info(f"  qubex-driver-quel1: {get_version('qubex-driver-quel1')}")
            logger.info(f"  qubex-pulse: {get_version('qubex-pulse')}")
            logger.info(f"  qubex-schema: {get_version('qubex-schema')}")
            logger.info(f"  qubex-simulator: {get_version('qubex-simulator')}")
