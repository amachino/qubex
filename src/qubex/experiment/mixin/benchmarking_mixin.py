...
from ...pulse import Blank, FlatTop, PulseArray, PulseSchedule, VirtualZ, Waveform
...
class BenchmarkingMixin(
        ...
        *,
        ...
        for target_group in target_groups:
            ...
            while True:
                ...
                for seed in seeds:
                    seed = int(seed)  # Ensure seed is an integer
                    basis_vals: dict[str, list[float]] = defaultdict(list)
                    for basis in ["X", "Y", "Z"]:
                        result = self.measure(
                        ...
                        for target, data in result.data.items():
                            iq = data.kerneled
                            basis_vals[target].append(
                                self.rabi_params[target].normalize(iq)
                            )
                    for target, values in basis_vals.items():
                        x_val, y_val, z_val = values
                        purity = 0.5 * (1 + x_val**2 + y_val**2 + z_val**2)
                        trial_data[target].append(purity)
    ...
    def stark_purity_experiment_1q(
        ...
        *,
        ...
        def purity_sequence(
            ...
            with PulseSchedule() as ps:
                purity_sequence = self.purity_sequence_1q(
                    target=target,
                    n=n_clifford,
                    x90=x90.get(target) if x90 else None,
                    interleaved_waveform=interleaved_waveform.get(target)
                ...
                stark_pulse = FlatTop(
                    duration=purity_sequence.duration + stark_ramptime * 2,
                    amplitude=stark_ampl,
                    tau=stark_ramptime,
                )
                ps.add(st, stark_pulse)
                ps.add(ins, Blank(stark_ramptime))
                ps.add(ins, purity_sequence)
            return ps
    ...
    def stark_ipurity_experiment(
        ...
        *,
        ...
        dimension = 2
        rb_result = self.stark_purity_experiment_1q(
            target=target,
            stark_amplitude=stark_amplitude,
            stark_ramptime=stark_ramptime,
            n_cliffords_range=n_cliffords_range,
        ...
        irb_result = self.stark_purity_experiment_1q(
            target=target,
            stark_amplitude=stark_amplitude,
            stark_ramptime=stark_ramptime,
            n_cliffords_range=n_cliffords_range,
    ...
    def stark_interleaved_purity_benchmarking(
        ...
        *,
        ...
        results = {}
        result = self.stark_ipurity_experiment(
            target=target,
            stark_amplitude=stark_amplitude,
            stark_ramptime=stark_ramptime,
            interleaved_clifford=interleaved_clifford,
