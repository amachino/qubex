...
class MeasurementService:
        ...
        *,
        ...
        for parent, child in entangle_steps:
            if isinstance(parent, int):
                parent = self.ctx.quantum_system.get_qubit(parent).label
            if isinstance(child, int):
                child = self.ctx.quantum_system.get_qubit(child).label
    ...
    def _measure_1d_cluster_state(
        ...
        *,
        ...
        for i in range(n_edges):
            ...
            for node in edge:
                node_spectators = self.ctx.get_spectators(node)
    ...
    def _canonical_edge(
        ...
        if isinstance(edge, str):
            ...
            if isinstance(qubits[0], int):
                qubit0 = self.ctx.quantum_system.get_qubit(qubits[0]).label
            else:
                qubit0 = qubits[0]
            if isinstance(qubits[1], int):
                qubit1 = self.ctx.quantum_system.get_qubit(qubits[1]).label
            else:
                qubit1 = qubits[1]
    ...
    def measure_bell_state_fidelities(
        ...
        *,
        ...
        if targets is None:
            target_pairs = self.ctx.cr_pairs
        else:
        ...
        *,
        ...
        all_edges = sorted(
            [
                ...
                and f"{pair[0]}-{pair[1]}" in self.ctx.calib_note.cr_params
            ]
        )
        ...
        if in_parallel:
            graph = self.ctx.quantum_system.chip_graph
            rounds = []
