...
class MeasurementService:
    ...
    async def execute_async(
        ...
        *,
        ...
        """Execute a schedule asynchronously and return multiple measurement results."""
        if mode is None:
            mode = "avg"
        if add_last_measurement is None:
            add_last_measurement = False
        ...
        with self.ctx.modified_frequencies(frequencies):
            result = await self.ctx.measurement.execute_async(
                schedule=schedule,
                mode=mode,
                shots=shots,
                interval=interval,
    ...
    async def measure_async(
        ...
        *,
        ...
        """Measure a sequence or schedule asynchronously and return results."""
        if mode is None:
            mode = "avg"
        if add_pump_pulses is None:
            add_pump_pulses = False
        ...
        with self.ctx.modified_frequencies(frequencies):
            result = await self.ctx.measurement.measure_async(
                waveforms=waveforms,
                mode=mode,
                shots=shots,
                interval=interval,
