...
def test_to_multiple_measure_result_returns_wrapped_result() -> None:
    """Given wrapped result, when converting to multiple result, then the same object is returned."""
    multiple = _make_multiple_measure_result()
    result = MeasurementResult(multiple=multiple)

    assert result.to_multiple_measure_result() is multiple
...
def test_to_measure_result_selects_requested_index() -> None:
    ...
    assert single.data["Q00"] is multiple.data["Q00"][1]
...
def test_to_measure_result_raises_for_invalid_index() -> None:
    ...
    result = MeasurementResult(multiple=_make_multiple_measure_result())
